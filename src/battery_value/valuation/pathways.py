...
def value_recycling(context: PathwayContext) -> PathwayValuation:
    ...
    if not processes:
        return _ineligible(
            ...
            context.currency,
            [
                "We have no recycling route on file for this battery type "
                f"({context.chemistry.label})."
            ],
        )
...
def value_reuse(context: PathwayContext) -> PathwayValuation:
    ...
    if health.soh < assumptions_config.minimum_soh:
        blockers.append(
            f"Its health is {health.soh:.0%}. Buyers want at least "
            f"{assumptions_config.minimum_soh:.0%} before fitting a used battery "
            "to a vehicle."
        )
    if health.age_years and health.age_years > assumptions_config.maximum_age_years:
        blockers.append(
            f"It is {health.age_years:.0f} years old. Buyers rarely take a "
            f"replacement battery over {assumptions_config.maximum_age_years:.0f} "
            "years old."
        )
    if not health.is_safe_for_reuse:
        blockers.append(
            "It is recorded as damaged or has an open safety warning, so it "
            "cannot be sold for use in a vehicle."
        )
    if blockers:
...
def value_parts_out(context: PathwayContext) -> PathwayValuation:
    ...
    if model is None or not model.components:
        blockers.append(
            "We could not identify the exact pack model, so we do not know which "
            "parts are inside it or what they sell for."
        )
        return _ineligible(Pathway.PARTS_OUT, context.currency, blockers)

    if health.condition.value == "thermal_event":
        blockers.append(
            "It has been through a fire or overheating event, so taking it apart "
            "is not safe."
        )
    if health.soh < config.minimum_soh:
        blockers.append(
            f"Its health is {health.soh:.0%}, too low for the modules to be worth "
            f"reselling (buyers want {config.minimum_soh:.0%} or more)."
        )
    if blockers:
        return _ineligible(Pathway.PARTS_OUT, context.currency, blockers)
...
def value_second_life(context: PathwayContext) -> PathwayValuation:
    ...
    if health.soh < minimum_soh:
        blockers.append(
            f"Its health is {health.soh:.0%}. Home and grid storage needs at "
            f"least {minimum_soh:.0%} to be worth rebuilding."
        )
    if health.age_years and health.age_years > config.maximum_age_years:
        blockers.append(
            f"It is {health.age_years:.0f} years old, past the point where "
            "rebuilding it for storage pays off."
        )
    if health.condition.blocks_reuse:
        blockers.append(
            "It is recorded as damaged, so it cannot safely be rebuilt into a "
            "storage battery."
        )
    if blockers:
    ...
    if life_ratio <= 0:
        return _ineligible(
            Pathway.SECOND_LIFE,
            context.currency,
            ["It has too little life left to be worth rebuilding for storage."],
        )
