...
class Pathway(str, Enum):
    ...
    @property
    def label(self) -> str:
        """Industry-standard name, used in reports and the CLI."""
        return {
    ...
    def friendly_label(self) -> str:
        """Plain-language name, for someone who simply owns the battery."""
        return {
            Pathway.REUSE: "Sell it as a working battery",
            Pathway.PARTS_OUT: "Take it apart and sell the parts",
            Pathway.SECOND_LIFE: "Reuse it for home or grid storage",
            Pathway.RECYCLING: "Recycle it for the metals inside",
        }[self]

    @property
    def plain_explanation(self) -> str:
        """One sentence on what actually happens to the battery on this route."""
        return {
            Pathway.REUSE: (
                "A garage fits it into another vehicle of the same model, so it "
                "carries on doing the job it was built for."
            ),
            Pathway.PARTS_OUT: (
                "A specialist splits the pack into modules and electronics and "
                "sells them separately, mostly to people repairing other batteries."
            ),
            Pathway.SECOND_LIFE: (
                "The pack is rebuilt into a stationary battery that stores solar "
                "power or cheap off-peak electricity."
            ),
            Pathway.RECYCLING: (
                "The pack is shredded and the nickel, cobalt, lithium and copper "
                "inside are recovered to make new batteries."
            ),
        }[self]
